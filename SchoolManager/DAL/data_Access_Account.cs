...
namespace SchoolManager.DAL
{
    public class data_Access_Account : data_Access_Base
    {
        ...
        public Accounts LoginWithToken(string token)
        {
            ...
            {
                using (SqlConnection conn = new SqlConnection(connection_String))
                using (SqlCommand cmd = new SqlCommand("sp_LoginWithToken", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@tokenRaw", token);
        ...
        public bool SaveRememberToken(int teacherId, string token, DateTime expires)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connection_String))
                using (SqlCommand cmd = new SqlCommand("sp_SaveRememberToken", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@tokenRaw", (object)token ?? DBNull.Value);
