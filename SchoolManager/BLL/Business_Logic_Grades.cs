...
namespace SchoolManager.BLL
{
    public class Business_Logic_Grades
    {
        ...
        {
            return dal.GetSubjectIdByName(name);
        }
        ...
        {
            DataTable rawData = dal.GetRawGrades(classId, semester);
            List<SubjectResultDTO> result = new List<SubjectResultDTO>();
            ...
            foreach (var group in grouped)
            {
                ...
                var subjectRows = group.Where(r => r["subject_name"]?.ToString() == subjectName);
                foreach (var row in subjectRows)
                {
                    string type = row["grade_period"]?.ToString();
                    double score = row["grade_score"] != DBNull.Value ? Convert.ToDouble(row["grade_score"]) : 0;

                    if (type == "Mid") dto.ScoreMid = score;
                    if (type == "Final") dto.ScoreFinal = score;
                }
                result.Add(dto);
            }
            return result;
        ...
        public AcademicReportDTO GetReportForSingleClass(int classId, DateTime startDate, DateTime endDate)
        {
            ...
            // --- KHỞI TẠO BẢNG PHÂN PHỐI ĐIỂM (7 Dòng) ---
            ...
            foreach (var group in studentGroups)
            {
                ...
                // Cộng tổng số chung (để tránh lỗi null nếu dùng ở chỗ khác)
                ...
                // --- TÍNH TOÁN CHO BẢNG 1 (Phân phối điểm) ---
                ...
                else bucketKey = roundedScore.ToString(); // 9, 8, 7, 6, 5
                ...
                // --- TÍNH TOÁN CHO BẢNG 2 (Chi tiết môn) ---
        ...
        {
            if (score >= 7) stat.CountT++;
            else if (score >= 5) stat.CountH++;
            else stat.CountC++;
        }
    }
}